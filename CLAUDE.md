...
The plugin ships its own PreToolUse hook (`plugins/1password/hooks/hooks.json` + `plugins/1password/scripts/block-fifo-read.sh`) that complements the upstream Bash-only validator. The companion hook auto-loads at plugin install (user scope, not project scope), matches `Read|Edit|MultiEdit|Write|NotebookEdit`, and denies any tool call whose target path is a named-pipe FIFO. Closes the leak path where Claude defaults to the Read tool for file inspection — which the upstream validator's Bash-only matcher cannot see. Track the upstream issue at <https://github.com/1Password/agent-hooks/issues/28> — when upstream supports non-Bash matchers, our companion can be removed.
